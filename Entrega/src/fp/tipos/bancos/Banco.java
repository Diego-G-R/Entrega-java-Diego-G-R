...
//BORRA ESTE COMENTARIO CUANDO ESCRIBAS CUENTAS y así no hay errores de compilacion
...
// Empleado más joven
	public Optional<Persona> empleadoMasJoven() {
		return this.empleados().todos().stream().map(x->x.persona()).min(Comparator.comparing((Persona x)-> x.edad()));
	}
// Número de cuentas de cada cliente
	public Map<String, Integer> numeroDeCuentasDeCliente() {
		return Stream2.groupingReduce(this.cuentas().todas().stream(), x-> x.dni, x->1, (x,y)->x+y);
	}
...
//		banco.test();
		...
		System.out.println(banco.prestamosDeCliente("71224840G"));
		System.out.println(banco.numeroDeCuentasDeCliente());
//		System.out.println(banco.cuentas());
	}
